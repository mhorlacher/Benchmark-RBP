...
parser.add_argument('--bed_files', type=str, help="List of bed files with binding sites")
parser.add_argument('--transcript_bed', type=str, help="Transcript bed file")
parser.add_argument('--bin_size', type=int, help="Bin size for transcript")
...
def main():
    ...
    with tempfile.NamedTemporaryFile(mode='r+') as temp:
        ...
        subprocess.call("bedtools intersect -wa -loj -a " +temp.name+ " -b "+ bed_files + """ | awk '{print $1"\t"$2"\t"$3"\t"$11"\t"$5"\t"$6}' | sort -k1,1 -k2,2n | bedtools merge -s -d -1 -c 4,5,6 -o collapse,distinct,distinct """,shell=True)
