...
parser = argparse.ArgumentParser(description="Data preprocessing for DeepRiPe")
...
def main():
    ...
    preds = model.predict([x_test,x_test_region])
    ...
    for index, rbp in enumerate(list(rbp_dict)):
        ...
        df_rbp = pd.DataFrame([["DeepRiPe", dataset, rbp, fold, model_type, sample_ids[i], x] for i,x in enumerate(predictions)])
